...
split_name = snakemake.params.tsv.split(".human")[0].split(snakemake.wildcards.sample)[
    -1
]
...
with open(snakemake.params.tsv, "w") as w_file:
    header = "\t".join(
        ["contig", "hit_genome", "pident", "evalue", "coverage", "description"]
    )
    w_file.write(f"{header}\n")

    for sub_blast in iterrator_on_blast_hsp(blast_out=blastn_file):
        ...
        if num_HSPs > 0:
            ...
            if coverage_blast >= coverage:
                line2write = f"{qseqid}\t{sseqid}\t{pident_blast}\t{evalue_blast}\t{coverage_blast}\t{description}\n"
                w_file.write(line2write)
                contig2remove.append(qseqid)

parser = SeqIO.parse(snakemake.input.fasta, "fasta")

path_input = Path(snakemake.input.fasta)

with open(snakemake.output.fasta, "wt") as w_file:
    with open(snakemake.output.translation_table, "wt") as translation_table:
        translation_table.write("old_name\tnew_name\n")
        for contig in parser:
            if contig.id not in contig2remove:
                tmp_contig = f"{snakemake.params.sample}--{contig.id}"
                translation_table.write(f"{contig.id}\t{tmp_contig}\n")
                contig.id = tmp_contig
